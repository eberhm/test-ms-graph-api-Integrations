data "aws_secretsmanager_secret" "ds_host_secret" {
  name = "${var.environment}/${var.name}-service/ds-host-secret"
}

data "aws_secretsmanager_secret_version" "ds_host_secret" {
  secret_id = data.aws_secretsmanager_secret.ds_host_secret.id
}
